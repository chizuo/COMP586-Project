                ...
                else
                {
                    ...
                    foreach (var schedule in scheduleRespone)
                    {
                        ...
                        foreach (var section in sectionRespone)
                        {
                            ...
                            var departmentRespone = db.dbDepartments.Single(d => d.code == section.dept);
                            Department department = new Department(departmentRespone.name, departmentRespone.code, null);

                            var professorRespone = db.dbPerson.Single(p => p.id == section.professor);
                            ...
                            foreach (var course in courseRespone)
                            {
                                ...
                                Section sectionObject = new Section(courseObject, section.sectionNumber, section.schoolTerm, section.schoolYear, section.enrollmentCap, section.waitListcap, section.waitListTotal, classDays, section.startTime, section.endTime, section.classLocation, null, section.classNote, null, professor);
                                student.Sections.Add(sectionObject);

                                for (int i = 0; i < classDays.Length; i++)
                                {
                                    Console.WriteLine(classDays[i]);
                                    student.Schedule[classDays[i]] = sections;
                                }
